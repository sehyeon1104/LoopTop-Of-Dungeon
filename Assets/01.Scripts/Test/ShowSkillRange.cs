...
public class ShowSkillRange : MonoBehaviour
{
    ...
    public void ActiveTrailPre()
    {
        ...
        for(int i = 0; i < 4; ++i)
        {
            trailsArr[i].transform.position = GameManager.Instance.Player.transform.position;
    ...
    public IEnumerator IEMoveTrail(GameObject trailObj, int count)
    {
        ...
        while (trailObj.transform.position != initPos)
        {
            ...
            if(Vector3.Distance(trailObj.transform.position, GameManager.Instance.Player.transform.position) < safeDistance)
            {
                if (!isSafe)
                {
                    Debug.Log("Safe");
